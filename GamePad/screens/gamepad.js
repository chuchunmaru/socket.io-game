...
export default class gamepad extends React.Component {
  ...
  componentDidMount() {
    ...
    this.setState({
      socket:
        io(`${this.props.route.params.url}`).on('connect', () => {
          this.state.socket.emit('tempest', 'deviceConnected')
        })
    })
        ...
        <View style={{ backgroundColor: 'black' }}>
          ...
          <View style={{ width: '100%', height: '100%', justifyContent: 'center', justifyContent: 'space-between', flexDirection: 'row', alignItems: 'center', padding: 10, }}>
            ...
            <View style={{ width: '50%', height: '100%', justifyContent: 'center', alignItems: 'center' }}>
              <TouchableOpacity onPress={() => this.state.socket.emit('tempest', 'upkeypressed')} >
              ...
              <View style={{ width: '100%', height: 120, justifyContent: 'space-between', flexDirection: 'row' }}>
                <TouchableOpacity onPress={() => this.state.socket.emit('tempest', 'leftkeypressed')} >
                  <View style={{ borderRadius: 10, borderWidth: 1, borderColor: 'black', backgroundColor: 'rgba(255,255,255,.7)', padding: 5, height: 120, width: 120, alignItems: 'center', justifyContent: 'center' }}>
                ...
                <TouchableOpacity onPress={() => this.state.socket.emit('tempest', 'rightkeypressed')} >
                  <View style={{ borderRadius: 10, borderWidth: 1, borderColor: 'black', backgroundColor: 'rgba(255,255,255,.7)', padding: 5, height: 120, width: 120, alignItems: 'center', justifyContent: 'center' }}>
              ...
              <TouchableOpacity onPress={() => this.state.socket.emit('tempest', 'downkeypressed')} >
                <View style={{ borderRadius: 10, borderWidth: 1, borderColor: 'black', backgroundColor: 'rgba(255,255,255,.7)', padding: 5, height: 120, width: 120, alignItems: 'center', justifyContent: 'center' }}>
            ...
            <View style={{ width: '40%', height: '800%', flexDirection: 'column', justifyContent: 'center', alignItems: 'center' }}>
              <TouchableOpacity onPress={() => this.state.socket.emit('tempest', 'Apressed')} >
              ...
              <TouchableOpacity onPress={() => this.state.socket.emit('tempest', 'Bpressed')} >
